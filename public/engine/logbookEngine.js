...
/** Haversine segment km; throws on bad data (no silent fixes). */
function logbookTripKmHaversine(fromLat, fromLng, toLat, toLng, fromLabel, toLabel) {
    let distanceKm = haversineDistance(fromLat, fromLng, toLat, toLng);
    const fromStr = String(fromLabel || '').trim();
    const toStr = String(toLabel || '').trim();
    if (distanceKm === 0 && fromStr !== toStr) {
        console.log('ZERO DISTANCE ERROR:', fromStr, toStr);
        throw new Error(`Invalid 0km trip between ${fromStr} and ${toStr}`);
    }
    if (distanceKm > 200) {
        throw new Error(`Unrealistic trip distance: ${distanceKm}km`);
    }
    return distanceKm;
}
...
function applySarsTripValidationFlags(entries) {
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        const distanceKm = Number(entry.businessKm);
        const distVal = Number.isFinite(distanceKm) ? distanceKm : 0;
        const fromProvince = normalizeTripProvince(entry.fromProvince);
        const toProvince = normalizeTripProvince(entry.toProvince);
        if (distanceKm === 0 && fromLocation !== toLocation) {
            entry.flag = 'INVALID_ZERO_DISTANCE';
        }
        if (distanceKm > 200) {
            entry.flag = 'INVALID_DISTANCE';
        }
...
function generateLogbookEntries(visits, vehicleOpeningKm, homeAddress, startDate, endDate, routes, manualEntries, workSaturdays, leaveDays, input) {
            ...
            if (isLeave) {
                ...
                if (dayVisits.length > 0) {
                    ...
                    if (typeof homeToFirstDistance !== 'number' || homeToFirstDistance < 0 || isNaN(homeToFirstDistance)) {
                        throw new Error(`Invalid distance for trip home → first visit: ${homeToFirstDistance}`);
                    }
    ...
    (input && input.mode === 'business')
        ...
        : 'Sales Visit';
                    ...
                    for (let i = 0; i < dayVisits.length - 1; i++) {
                        ...
                        if (
                            !Number.isFinite(fromVisit.lat) || !Number.isFinite(fromVisit.lng) ||
                            !Number.isFinite(toVisit.lat) || !Number.isFinite(toVisit.lng)
                        ) {
                            throw new Error(`Missing coordinates for ${fromVisit.customer} → ${toVisit.customer}`);
                        }
                        ...
                        if (typeof tripDistance !== 'number' || tripDistance < 0 || isNaN(tripDistance)) {
                            throw new Error(`Invalid distance for visit → visit trip: ${tripDistance}`);
                        }
    ...
    (input && input.mode === 'business')
        ...
        : `${toVisit.reason || 'Sales Visit'} – ${toVisit.customer || ''}`;
                    ...
                    if (!Number.isFinite(lastVisit.lat) || !Number.isFinite(lastVisit.lng)) {
                        throw new Error(`Missing coordinates for ${lastVisit.customer}`);
                    }
                    ...
                    if (typeof lastToHomeDistance !== 'number' || lastToHomeDistance < 0 || isNaN(lastToHomeDistance)) {
                        throw new Error(`Invalid distance for return trip: ${lastToHomeDistance}`);
                    }
...
async function runLogbookEngine(input) {
    ...
    if (homeLat == null || homeLng == null || typeof homeLat !== 'number' || typeof homeLng !== 'number' || isNaN(homeLat) || isNaN(homeLng)) {
        throw new Error('homeLat and homeLng are required numbers for distance calculation');
    }
    ...
    for (let vi = 0; vi < visits.length; vi++) {
        ...
        if (!Number.isFinite(visit.lat) || !Number.isFinite(visit.lng)) {
            console.log('FAILED VISIT:', visit.fullAddress);
            console.error('FAILED VISIT DEBUG:', {
                customer: visit.customer,
                fullAddress: visit.fullAddress,
                lat: visit.lat,
                lng: visit.lng
            });
            throw new Error(`Missing coordinates for ${visit.customer}`);
        }
    }
    ...
    const totalTravelKm = entries.reduce((sum, entry) => {
    ...
    const formattedEntries = entries.map((entry) => {
        ...
        if (Object.prototype.hasOwnProperty.call(entry, 'distanceKm')) {
            formattedEntry.distanceKm = round2(entry.distanceKm);
        }
